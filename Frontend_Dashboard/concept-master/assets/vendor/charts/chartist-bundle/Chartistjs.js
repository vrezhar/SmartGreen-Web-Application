...
//get requesti from to sahmanneryasync 
  let now = dateToUnixtime();
  let anhourago = now - 60*60;

  async function get_from_api (url) 
  {
     ...
     else humidity.push(data); 
        return humidity;
     time.push(new Date());
  }

 let url = "http://smartgreen.cc/demo/api/api_GET.php?getSensorData&sensor_id=1&limit=1"
    ...
    let promise = new Promise(
        ...
        { 
            setInterval(
                () =>
                { 
                    chart_1.load({ labels: ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'],
                series: [
                    [12, 9, 7, 8, 5],
                    [2, 1, 3.5, 7, 3],
                    [1, 3, 4, 5, 6]
                   ...
                   /* get_from_api(url).then(chart_1.load({labels: time,
                series: [
                    temperature, humidity
                ]}));*/ //alert("testing...")//fetchasync(url)-i veradardzraci het inch vor ban anel
                }, 3000)
...
(function(window, document, $, undefined) {
    ...
    bindto: '.ct-chart-line-one',
    data: {
      columns: [
        ['Humidity', 30, 200, 100, 400, 150, 250],
        ['Temperature', 50, 20, 10, 40, 15, 25]
      ],
      axes: {
        data2: 'y2'
      },
...
setInterval(()=>{chart.load({
  columns: [
   ['Humidity', get_from_api(url)],
   ['Temperature', get_from_api(url)]
  ]
});
 }, 5000);

        /* if ($('.ct-chart-line-one').length) {
            var chart_1 = new Chartist.Line('.ct-chart-line-one', {
                labels: time,
                series: [
                    [12, 9, 7, 8, 5],
                    [2, 1, 3.5, 7, 3]
...
if ($('.ct-chart-line').length) {
        ...
        if ($('.ct-chart-scatter-bar').length) {
            new Chartist.Bar('.ct-chart-scatter-bar', {
               ...
               if ($('.ct-chart-scatter-bar-one').length) {
            new Chartist.Bar('.ct-chart-scatter-bar-one', {
                labels: ['Q1', 'Q2', 'Q3', 'Q4'],
                series: [
                    [800000, 1200000, 1400000, 1300000],
                ]
